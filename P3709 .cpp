...
int N , M , Cnt1[MAXN] , Cnt2[MAXN] , Unit , Ans[MAXN] , Res , Num[MAXN] , Num2[MAXN];
...
void Add(int x) {
	Cnt1[Cnt2[Num[x]]]--;
	Cnt1[++Cnt2[Num[x]]]++;
	Res = max(Res , Cnt2[Num[x]]);
}

void Del(int x) {
	if(Cnt2[Num[x]] == Res && Cnt1[Cnt2[Num[x]]] == 1) Res--;
	Cnt1[Cnt2[Num[x]]]--; 
  	Cnt1[--Cnt2[Num[x]]]++; 
}
...
int main() {
	...
	int lxl = unique(Num2 + 1 , Num2 + 1 + N) - Num2 - 1;
	for(int i = 1; i <= N; i++) {
		Num[i] = lower_bound(Num2 + 1 , Num2 + 1 + lxl , Num[i]) - Num2;
	}
	for(int i = 1; i <= M; i++) {
		cin(Ask[i].l , Ask[i].r);
		Ask[i].id = i;
